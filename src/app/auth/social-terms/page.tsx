...
export default function SocialTermsPage() {
  ...
  // 모달에서 "동의" 버튼을 누르면 체크박스 활성화 + 모달 닫힘
  ...
  const handleAgree = async () => {
    ...
    try {
      ...
      router.push('/'); // 약관 동의 및 세션 완료 시 메인, 또는 원하는 경로로 이동
    } catch (err: any) {
  ...
  return (
    <div className="app-container flex flex-col bg-gray-50 min-h-screen">
      ...
      <main 
        className="
          flex-1 flex flex-col items-center px-6 pt-4 md:pt-8 lg:pt-10 overflow-y-auto justify-start
        "
      >
        <div className="w-full max-w-sm">
          ...
          <div 
            className="
              mb-4 bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-2 r
              ounded-x1 text-center text-sm font-medium items-center
            "
          >
        ...
        >
          ...
          {error && (
            <div 
              className="
                bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm mt-3 text-center
              "
            >
              {error}
            </div>
