        ...
        <div className="app-container flex flex-col min-h-screen bg-gray-50">
          ...
          <main className="flex-1 flex flex-col items-center px-6 pt-4 pb-10 overflow-y-auto">
            <div className="w-full max-w-sm mt-4">
              <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg mb-4 shadow">
                <div className="flex items-center">
                  <svg className="h-5 w-5 text-yellow-400 mr-2" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                ...
                <button
                  ...
                  className="mt-4 w-full py-2 px-3 rounded-lg bg-yellow-400 text-white font-semibold hover:bg-yellow-500 transition"
                >
                  우산 대여하기
                </button>
                <button
                  onClick={() => router.push('/dashboard')}
                  className="mt-2 w-full py-2 px-3 rounded-lg border font-medium text-yellow-900 bg-white shadow hover:bg-yellow-100 transition"
                >
                  대시보드로 돌아가기
                </button>
              </div>
        ...
        <main className="flex-1 flex flex-col items-center px-6 pt-4 pb-10 overflow-y-auto">
          <div className="w-full max-w-sm mt-4 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow p-6 space-y-3">
              <h3 className="text-lg font-bold text-gray-900 mb-2">현재 대여 중인 우산</h3>
              <div className="text-sm text-gray-600 mb-3">반납할 우산 정보입니다.</div>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="font-medium text-gray-500">우산 코드</span>
                  <span className="text-gray-900">{activeRental.umbrella?.code || '정보 없음'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium text-gray-500">대여 시작</span>
                  <span className="text-gray-900">{new Date(activeRental.rentalStart).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium text-gray-500">대여소</span>
                  <span className="text-gray-900">
                    {stations.find(s => s.id === activeRental.rentalStationId)?.name || '정보 없음'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium text-gray-500">보증금</span>
                  <span className="text-gray-900">{activeRental.depositAmount} 츄르</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-2">반납할 대여소 선택</h3>
              <div className="text-sm text-gray-600 mb-3">우산을 반납할 대여소를 선택하세요.</div>
              <ul className="divide-y divide-gray-200 max-h-64 overflow-y-auto">
                {stations.map((station) => (
                  <li key={station.id}>
                    <button
                      onClick={() => setSelectedStation(station)}
                      className={`w-full flex flex-col text-left px-4 py-3 hover:bg-indigo-50 rounded-md ${
            ...
            {error && (
              <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mt-3 text-sm">
                {error}
              </div>
            )}
          </div>
